...
void Nrf24_printDetails(NRF24_t * dev)
{
	...
	Nrf24_print_byte_register(dev, "EN_AA\t", EN_AA, 1);// default ACK open 
...
// disable ACK
void Nrf24_disableAA(NRF24_t *dev) {
    uint8_t off = 0x00;
    Nrf24_writeRegister(EN_AA, &off, 1);  // Disable Auto Acknowledgment on all pipes
}
void Nrf24_SetCRCLength(NRF24_t *dev, uint8_t mode)
{
    uint8_t config;
    Nrf24_readRegister(CONFIG, &config, 1);

    if (mode == RF24_CRC_DISABLED) {
        config &= ~(1 << EN_CRC); // Clear EN_CRC to disable CRC
    } else if (mode == RF24_CRC_8) {
        config |= (1 << EN_CRC);
        config &= ~(1 << CRCO); // Clear CRCO for 8-bit
    } else if (mode == RF24_CRC_16) {
        config |= (1 << EN_CRC);
        config |= (1 << CRCO); // Set CRCO for 16-bit
    }
